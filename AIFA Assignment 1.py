...
if __name__=='__main__':
    ...
    n = int (input())
    arr=[[] for i in range(n)] ## 0 - indexed
    dist= [[9999 for i in range(n)]for j in range(n)]
    print ('Now, for each nodes please input the Vertices and the Edge Costs')
    for _ in range(n):
        a = list(map(int,input().split()))
        f ,t, ed = a  ## Input the 2 nodes and the Edges cost
        arr[f].append(t) ##-> arr is the Adjcency List
        arr[t].append(f)
        dist[f][t]=ed    ##-> Dist is the Adjacency Matrix
        dist[t][f]=ed
    x = all_pairs_shortest_path(dist)
    print (mafp(ev,arr,x)) 
